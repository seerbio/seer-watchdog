...
if "%sourceDir:EXP=%" neq "%sourceDir%" (
    ...
    set "aws_region=eu-central-1"  :: Change this if the region is different for GER
) else (
...
python C:\seer-scripts\watchdog3\seer_watchdog.py --aws_access_key_id "%aws_access_key_id%" --aws_secret_access_key "%aws_secret_access_key%" --aws-region "%aws_region%" --source "%sourceDir%" --bucket seer-internalms-eu --instrument Thermo --destination S3 --log_group ms_data_log_group --log_stream ms_data_log_stream >> %watchdog_batch_script_log% 2>&1
